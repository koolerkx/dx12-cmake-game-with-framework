...
namespace {
constexpr size_t kQueueBatchSize = 2048;
constexpr std::chrono::milliseconds kWorkerWaitMs(25);
}  // namespace
    ...
    while (true) {
      {
        ...
        s.cv.wait_for(lock, kWorkerWaitMs, [&]() { return !s.queue.empty() || !s.running.load(); });

        batch.clear();
        while (!s.queue.empty() && batch.size() < kQueueBatchSize) {
...
void Logger::Flush() {
  ...
  drained.reserve(kQueueBatchSize);

  {
    std::scoped_lock lock(state.queue_mutex);
    while (!state.queue.empty() && drained.size() < kQueueBatchSize) {
